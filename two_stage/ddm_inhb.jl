...
function value_function(v1::Vector{Float64}, v2::Vector{Float64}, weight=1.0)
    ...
    if length(v2) == 4
        ...
        v[1:2] .= weight1 * v1[1] .+ weight2 * v2[1:2]
        v[3:4] .= weight1 * v1[2] .+ weight2 * v2[3:4]
    elseif length(v2) == 3
        ...
        v[2:3] .= weight1 * v1[2] .+ weight2 * v2[2:3]
    else
...
# Adjusted apply_inhibition! function according to the provided model
function apply_inhibition!(dv, dv_alt, κ, β, N)
    copy!(dv_alt, dv)
    for i in eachindex(dv)
        inhibition_sum = sum(dv_alt[j] for j in eachindex(dv) if j != i)
        dv[i] += (- κ * dv[i] - β * inhibition_sum)
    end
end

# Simulate two-stage decision making with the modified inhibition model
function simulate_two_stage(model::DDM_inhb, v1::Vector{Float64}, v2::Vector{Float64}; maxt=1000, logger=(dv, stage, t) -> nothing)
    ...
    for t in 1:maxt
        ...
        # apply_inhibition!(dv, dv_alt, κ, β, N)
